...
| File | Focus |
|------|-------|
| `tests/generate-route.test.ts` | `POST /api/generate` route behavior: API key guard, invalid JSON, size limits, rate limiting headers, provider error mapping, malformed model JSON |
...
- Route tests mock `openai` SDK classes and completion calls to force deterministic failure/success paths.
