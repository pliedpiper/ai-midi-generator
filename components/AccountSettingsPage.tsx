...
const AccountSettingsPage: React.FC<AccountSettingsPageProps> = ({
  ...
  const [keyConfigured, setKeyConfigured] = React.useState(
    initialOpenRouterConfigured,
  );
  ...
  const [deleteAllError, setDeleteAllError] = React.useState<string | null>(
    null,
  );
  const [deleteAllMessage, setDeleteAllMessage] = React.useState<string | null>(
    null,
  );
  ...
  const handleDeleteAllGenerations = async () => {
    ...
    const confirmed = window.confirm(
      "Delete all saved generations? This cannot be undone.",
    );
    if (!confirmed) {
      return;
    }
  ...
  const formattedUpdatedAt = keyUpdatedAt
      ...
      <main className="flex-1 px-4 py-8 md:px-10 md:py-10">
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <section className="bg-surface-800 border border-surface-600 rounded-lg p-6">
            ...
            {apiKeyMessage && (
              <p className="mt-3 text-sm text-green-400">{apiKeyMessage}</p>
            )}
          </section>
          ...
          <section className="bg-surface-800 border border-surface-600 rounded-lg p-6">
            ...
            {deleteAllError && (
              <p className="mt-3 text-sm text-red-400">{deleteAllError}</p>
            )}
            {deleteAllMessage && (
              <p className="mt-3 text-sm text-green-400">{deleteAllMessage}</p>
            )}
          </section>
          ...
          <section className="bg-red-500/5 border border-red-500/30 rounded-lg p-6">
            ...
            <p className="text-sm text-red-200/80 mt-1">
              This permanently removes your account, OpenRouter key, and saved
              generations.
            </p>
